...
pub struct Ruft {
    ...
    applied_receiver: Option<mpsc::Receiver<ApplyMsg>>, // 暴露给调用者的 Apply Receiver。
}

// Raft 日志条目：记录产生该命令的任期和要应用到状态机的命令字节。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct LogEntry {
    pub term: u64,        // 写入该日志的任期。
    pub command: Vec<u8>, // 状态机命令。
}
...
impl Ruft {
    ...
    fn _boardcast_append_entries(&self) {
        ...
        let snapshot_calls = snapshot_args_list
            .into_iter()
            .map(|(id, args)| (id, client.call_install_snapshot(id, ctx.clone(), args)))
            .collect::<Vec<_>>();
        let append_calls = append_args_list
            .into_iter()
            .map(|(id, args)| (id, client.call_append_entries(id, ctx.clone(), args)))
            .collect::<Vec<_>>();

        tokio::spawn(async move {
            for (id, call) in snapshot_calls {
                match call.await {
                    Ok(reply) => {
    ...
    async fn _on_vote_result(&mut self, success: bool, new_term: u64) {
        // 即使该结果来自过期选举，更高任期也必须立即被采用。
        if new_term > self.current_term {
            if let Err(err) = self._refresh_term(new_term) {
                error!("Error persisting higher term from VoteResult: {err}");
            }
            return;
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn candidate_adopts_higher_term_from_vote_result() {
        let mut node = new_test_node(1, vec![1, 2, 3]);
        node.role = Role::Candidate;
        node.current_term = 2;
        node.voted_for = Some(1);

        node._on_vote_result(false, 3).await;

        assert_eq!(node.role, Role::Follower);
        assert_eq!(node.current_term, 3);
        assert_eq!(node.voted_for, None);
        let persisted = node.storage.load().expect("load storage");
        assert_eq!(persisted.hard_state.current_term, 3);
        assert_eq!(persisted.hard_state.voted_for, None);
    }
